...
static void auto_install_desktop_file(void) {
    const char *home = getenv("HOME");
    if (!home) return;
    
    char icon_dir[1024];
    snprintf(icon_dir, sizeof(icon_dir), "%s/.local", home);
    mkdir(icon_dir, 0755);
    snprintf(icon_dir, sizeof(icon_dir), "%s/.local/share", home);
    mkdir(icon_dir, 0755);
    snprintf(icon_dir, sizeof(icon_dir), "%s/.local/share/icons", home);
    mkdir(icon_dir, 0755);
    
    char icon_path[2048];
    snprintf(icon_path, sizeof(icon_path), "%s/iv_encrypt.png", icon_dir);
    
    if (access(icon_path, F_OK) != 0) {
        FILE *f = fopen(icon_path, "wb");
        if (f) {
            fwrite(favicon_png, 1, favicon_png_len, f);
            fclose(f);
        }
    }
    ...
    snprintf(desktop_path, sizeof(desktop_path), "%s/iv_encrypt.desktop", app_dir);
    ...
    if (access(desktop_path, F_OK) != 0) {
        ...
        if (len > 0) {
            ...
            if (f) {
                fprintf(f, "[Desktop Entry]\nName=IV_Encrypt\nComment=Steganography and Metadata\n");
